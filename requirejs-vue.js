...
define(['module'], function (module) {
  ...
  if (typeof window !== 'undefined' && window.document) {
    // browser-side
    if (typeof XMLHttpRequest === 'undefined') { throw new Error('XMLHttpRequest not available') }

    fetchContent = function (url, callback) {
      var xhr = new XMLHttpRequest()
      xhr.open('GET', url, true)
      xhr.onreadystatechange = function () {
        if (xhr.readyState === 4 && this.status < 400) { callback(xhr.responseText) }
      }
      xhr.send()
    }
  } else {
    ...
    if (!fs || !fs.readFileSync) { throw new Error(module.id + ': Unsupported platform') }
  ...
  var extractor = {
    ...
    _wrapped_content: function (text, tagname, options) {
      ...
      if (options.lastIndex) { end = text.lastIndexOf('</' + tagname + '>') }

      text = text.substring(start, end)

      if (!options.whitespaces) { text = text.replace(/\n/g, ' ').replace(/\s{2,}/g, ' ') }

      if (options.escape) { text = text.replace(/(['\\])/g, '\\$1') }
   ...
   * @param style
   */
  var processStyles = function (style, name) {
    if (!style || !style.trim().length) { return }
    if (masterConfig.isBuild || typeof document === 'undefined') {
      styleBlob[name] = style
      return
    }
  ...
  var parse = function (text, name) {
    ...
    processStyles(extractor.style(text), name)
  ...
  return {
    ...
    write: function (plugin, module, write) {
      ...
      if (styleBlob[module] && styleBlob[module] !== '') {
        write('(function() {var e = document.createElement(\'style\'); e.type = \'text/css\'; e.appendChild(document.createTextNode(\'' +
          styleBlob[module].replace(/'/g, "\\'") +
          '\')); document.body.appendChild(e);}())'
        )
      }
    }
